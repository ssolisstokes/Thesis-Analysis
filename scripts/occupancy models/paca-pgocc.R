...
gm.inits <- list(alpha = 0, 
                   beta = 0, 
                   z = apply(y, 1, max, na.rm = TRUE))

gm.priors <- list(alpha.normal = list(mean = 0, var = 2.72), 
...
out <- PGOcc(occ.formula = global.occ.formula,
...
# probability scale of the detection intercept
plogis(-3.2972) # = 0.035 
...
# comparing global model to constant occupancy 
...
out.k.fold <- PGOcc(occ.formula = global.occ.formula,
                    det.formula = global.det.formula, 
                    data = list(
                      y = y,
                      occ.covs = occ_df,   
