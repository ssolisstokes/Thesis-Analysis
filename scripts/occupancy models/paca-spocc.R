...
# detection formula
global.det.formula <- ~ rock + I(t_air^2) + t_air + I(vpd^2) + vpd + rock + wind + I(t_gr^2) + t_gr
# model type
...
gm.inits <- list(
  ...
  sigma.sq  = 1.5,                       # spatial variance (relatively small bc all in PC)
...
ppc.sp.out2 <- ppcOcc(out.sp, fit.stat = 'freeman-tukey', group = 1)
