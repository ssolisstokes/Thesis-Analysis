...
This document serves as a chronology of the analysis I've completed to date for the 2025 Gila monster occupancy project. I've excluded a decent amount of the code from this version of the document for a smoother read-through, if you'd like to see the entire code [visit this page](https://ssolisstokes.github.io/Thesis-Analysis/htmls/thesis-update-with-code.html).

# Summary Tables

This section contains summary tables relating to the survey design and effort (transect lengths and survey hours).

```{r summary libraries, include=FALSE}
library(dplyr)
library(tidyr)
library(readr)
library(janitor)
...
```{r transect length, include=FALSE}
...
```{r site length summary, echo=FALSE}
# adapted from: occ_transect-distance.R
...
```{r transect length summary, echo=FALSE}
...
## Overall Effort
```{r overall effort summary, echo=FALSE}
...
```{r density libraries, echo=FALSE}
library(dplyr)
library(tidyr)
library(readr)
library(janitor)
...
```{r all site var density plot, echo=FALSE}
...
```{r detection vs non-detection density plot, echo=FALSE}
...
```{r pgocc-libraries, include=FALSE}
library(dplyr)
library(tidyr)
library(spOccupancy)
library(car)
...
```{r prep covariates for pgOcc, include=FALSE}
...
```{r MCMC trace plots global, echo=TRUE}
...
### Freeman-Tukey Test
```{r freeman tukey global, echo=FALSE}
...
```{r freeman tukey global plot sites, include=FALSE}
# visualization 
...
## Occupancy Variables
```{r collinearity-occupancy vars, echo=FALSE}
...
## Detection Variables
```{r collinearity-detection vars, echo=FALSE}
...
## Candidate Models
```{r occupancy candidate models, include=FALSE}
...
## WAIC Table
```{r occupancy waic tbl, echo=FALSE}
...
## Variable Importance
```{r occupancy variable importance, echo=FALSE}
...
```{r model avg coef occupancy, echo=FALSE}
...
```{r occupancy formula, include=FALSE}
...
## Candidate Models
```{r detection candidate models, include=FALSE}
...
## WAIC Table
```{r detection waic tbl, echo=FALSE}
...
## Variable Importance
```{r detection variable importance, echo=FALSE}
# model weights as objects
...
## Model-Averaged Coefficients
```{r model avg coef detection, echo=FALSE}
# dm1 posterior means
...
```{r detection formula, include=FALSE}
...
```{r bootstrap libraries, include=FALSE}
library(dplyr)
library(tidyr)
library(purrr)
library(rsample)
...
```{r bootstrap set up & resampling, include=FALSE}
...
```{r bs confidence tbl, echo=FALSE}
...
## Bootstrap Boxplots
```{r bs raw data boxplots, echo=FALSE}
...
```{r adding air temp det models, include=FALSE}
...
## WAIC Table
```{r updated detection WAIC tbl, echo=FALSE}
dm1.waic <- waicOcc(dm1)
dm2.waic <- waicOcc(dm2)
dm3.waic <- waicOcc(dm3)
dm4.waic <- waicOcc(dm4)
...
## Variable Importance 
```{r updated detection variable importance, echo=FALSE}
# model weights as objects
dm1.weight <- dm.waic_tbl$weight[dm.waic_tbl$model == "dm1"]
dm2.weight <- dm.waic_tbl$weight[dm.waic_tbl$model == "dm2"]
dm3.weight <- dm.waic_tbl$weight[dm.waic_tbl$model == "dm3"]
...
## Model-averaged Coefficients
```{r updated detection model averaged coefficients, echo=FALSE}
# dm1 posterior means
dm1.mat <- as.matrix(dm1$alpha.samples)
dm1.means <- colMeans(dm1.mat)
...
```{r tested best-fit detection formulas, include=FALSE}
...
```{r best-fit model selection, include=FALSE}
...
## Best-fit WAIC Table
```{r best-fit waic table, echo=FALSE}
...
```{r final best fit model, echo=FALSE}
fit.detection.formula <- ~ rock + wind + t_air + I(t_air^2)
best.fit_model<- PGOcc(occ.formula = fit.occupancy.formula,
                          det.formula = fit.detection.formula, 
                          data = list(
...
```{r predicted occ and detection prob, echo=FALSE}
...
## Freeman-Tukey Test
```{r best fit freeman test sites, echo=FALSE}
...
```{r best fit freeman test reps, echo=FALSE}
# create df of group 1 (site); color for actual data set
...
## Chi-Squared Test 
```{r chi squared test best fit, echo=FALSE}
...
```{r habitat class libraries, include=FALSE}
library(dplyr)
library(tidyr)
library(readr)
library(janitor)
...
```{r habitat class set up, include=FALSE}
...
```{r habitat class model summary, echo=FALSE}
# habitat formula
...
## Probability Table
```{r habitat probability summary table, echo=FALSE}
...
```{r habitat class probabilty ggplot, echo=FALSE}
...
## Comparison Table
```{r habitat class comparison table, echo=FALSE}
...
```{r habitat class comparison plot, echo=FALSE}
